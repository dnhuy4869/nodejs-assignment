...
const EditForm = () => {
    ...
    const getProductById = async (id) => {
        try {
            const res = await axios.get(`http://127.0.0.1:8000/product/get-one/${id}`);
            //console.log(res.data);
            return res.data;
        }
        catch (err) {
    ...
    const [currProduct, setCurrProduct] = useState({
        name: "",
        price: 0,
        idCategory: allCategories[0] ? allCategories[0]._id : "",
        image: "",
    });

    useEffect(() => {
        (async () => {
            const cate = await getProductById(id);
            if (!cate) {
                navigate("/admin/products", { replace: true });
            }
    ...
    const formik = useFormik({
        ...
        initialValues: {
            ...
            category: currProduct.idCategory,
            images: [],
        },
        ...
        onSubmit: async (values) => {
            ...
            try {
                const res = await axios.put(`http://127.0.0.1:8000/product/edit-one/${id}`,
                    data,
                    {
                        headers: {
